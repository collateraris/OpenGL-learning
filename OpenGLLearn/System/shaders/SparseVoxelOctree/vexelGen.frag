...
layout (binding = 0, offset = 0) uniform atomic_uint aVoxelCounter;
...
void ComputeCriticalPointDirections(const in  vec3 normal, const in vec3 d)
{
    dmin.x = (normal.x > 0.0f) ? -d.x : d.x;
    dmin.y = (normal.y > 0.0f) ? -d.y : d.y;
}

bool TriangleBoxOverlap( const in vec3 voxCenter, const in vec3 d)
{
    vec2 minMaxValue;
    float p0, p1, r;
...
void main()
{
    ...
    ComputeCriticalPointDirections(face_normal, diag);
    ...
    for ( int z = min(z_index_cmin,z_index_cmax); z <= max(z_index_cmin,z_index_cmax); ++z)
        if (TriangleBoxOverlap(vec3(c, -2.f*(z+0.5f)*diag.z+1.f), diag))
        {
            if (axis == 1)
        ...
        {    
            ...
            int index = int(atomicCounterIncrement(aVoxelCounter));
            //index = uint(tc.z * res.y* res.x + tc.y * res.x + tc.x); //only for testing!
            aVoxelFragmentList[index] = vec4(tc/vec3(res),1); 

            vec3 color = tc/vec3(res); //TODO put color in here!
            aVoxelFragmentColor[index] = vec4(color, 1.0f);
