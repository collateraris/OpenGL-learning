...
vec3 atmosphere(in vec3 rayDir, in vec3 rayOrigin, in vec3 sunPos, float sunIntensity);

vec2 rsi(in vec3 r0, in vec3 rd, float sr); 

void main()
{	
	vec3 color = vec3(0.0);
			
	vec3 dir = normalize(vec3(gl_FragCoord.xy - uResolution * 0.5, 
                                  -uResolution.y / tan(M_PI * 0.167)));
   ...
   // if (worldDir.y > 0.) 
	{
		color = atmosphere(normalize(vPosition), vec3(0,6372e3,0), uSunPos, 150.0);
	}
