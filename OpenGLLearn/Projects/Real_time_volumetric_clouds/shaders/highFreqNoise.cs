...
void main()
{
    vec3 st128 = gl_GlobalInvocationID.xyz;
    ivec3 pixel128 = ivec3(st128);
    ivec3 pixel64 = pixel128 / 2;
    ivec3 pixel32 = pixel64 / 2;
    vec4 noise = vec4(1.);
    ...
    float finalValue = remap(noise.x, (noise.y * 0.625 + noise.z * 0.25 + noise.w * 0.125) - 1, 1, 0, 1);

    imageStore(outTexture, pixel128, vec4(finalValue));
}
