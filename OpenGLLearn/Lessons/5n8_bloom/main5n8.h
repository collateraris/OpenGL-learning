...
namespace lesson_5n8
{
	...
	int lesson_main()
	{
		...
		if (!lightingShader.Init("Lessons/5n8_bloom/shaders/lighting.vs", "Lessons/5n8_bloom/shaders/lighting.fs")) return -1;

		lesson_1n5::CShader hdrShader;
		if (!hdrShader.Init("Lessons/5n8_bloom/shaders/hdr.vs", "Lessons/5n8_bloom/shaders/hdr.fs")) return -1;
		...
		unsigned int containerTexture = lesson_3n1::CLoadTexture::LoadGammaTexture("content/tex/container2.png");
		...
		while (!glfwWindowShouldClose(window))
		{
			...
			cubeLightShader.Use();
			cubeLightShader.setMatrix4fv("view", view);
			for (unsigned int i = 0; i < lightPositions.size(); i++)
			{
				model = glm::mat4(1.0f);
				model = glm::translate(model, glm::vec3(lightPositions[i]));
				model = glm::scale(model, glm::vec3(0.25f));
				cubeLightShader.setMatrix4fv("model", model);
				cubeLightShader.setVec3f("lightColor", lightColors[i]);
				renderCube();
			}
